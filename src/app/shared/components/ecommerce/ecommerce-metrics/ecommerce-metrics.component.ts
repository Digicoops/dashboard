...
export class EcommerceMetricsComponent implements OnInit {
  ...
  async loadProducersCount() {
    try {
      ...
      if (result.error) {
        ...
        this.animatedProducersCount = 0;
      } else {
        this.producersCount = result.producers.length;
        this.animateCount(this.producersCount);
  ...
  private animateCount(target: number) {
    ...
    const timer = setInterval(() => {
      current += step;
      if (current >= target) {
        this.animatedProducersCount = target;
        clearInterval(timer);
      } else {
        this.animatedProducersCount = Math.floor(current);
      }
    }, duration / steps);
  }
  ...
  private calculatePercentage() {
    // Ici vous pouvez calculer un pourcentage basé sur vos données
    // Par exemple, comparer avec le mois précédent ou utiliser une valeur fixe
